...
| `nome_en` / `nome_fr` / `nome_es` / `nome_de` | String | Traduzioni statiche di `nome_it` (4 settembre 2026, vedi `DECISIONI.md` nel repo principale, blocco "Ricette: traduzione EN/FR/ES/DE") — sempre presenti per tutte le 446 ricette, naturali non letterali, stesso criterio di `nome_it`/`procedimento_it`. Non generate da un servizio di traduzione automatica: traduzione diretta, poi validata per parità di lunghezza con `procedimento_it` |
...
| `procedimento_en` | [String] | Sempre presente per tutte le 446 ricette dal 4 settembre 2026 (vedi `DECISIONI.md`, blocco "Ricette: traduzione EN/FR/ES/DE"). Per le ricette la cui fonte originale era già in inglese (`kaggle-foodcom`, `wikibooks-en`, 211/446) è il testo originale inglese, non una ritraduzione. Per le restanti 235 (fonte italiana o senza fonte testuale) è una traduzione diretta da `procedimento_it`. L'app Android mostra `procedimento_en` al posto di `procedimento_it` solo se la lingua di sistema del dispositivo è inglese |
| `procedimento_fr` / `procedimento_es` / `procedimento_de` | [String] | Sempre traduzioni dirette da `procedimento_it` (nessuna fonte originale possibile in queste lingue) — sempre presenti per tutte le 446 ricette dal 4 settembre 2026, stesso numero di step di `procedimento_it` (parità di lunghezza validata programmaticamente) |
